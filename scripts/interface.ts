function renderCard(card: Card, faceUp: boolean) {
  ...
  if (card.suit == "H" || card.suit == "D") color = "red";
...
function renderHeader(casino: Casino) {
  ...
  elTopBar.innerHTML = `<div class="userInfo">
    <div class="toolbar">
        ...
        <p>
            <span>
                <a id="showInfoBox" href="#">${
                  casino.game.players[casino.game.turn].name
                }</a>'s turn |</span>
...
function renderTable(casino: Casino) {
  ...
  el.appendChild(
    renderDeck(
      casino.game.discardPile.cards[casino.game.discardPile.cards.length - 1]
    )
  );
...
function renderBettingMenu(user: User, submitCb: (amount: number) => void) {
  ...
  el.innerHTML = `
    ...
    <hr>
        ...
        <p>The bet has to be less than your total holdings ($${
          user.moneyRemaining
        })</p>
        <p class="error"></p>
    `;
  ...
  btn.addEventListener("click", function() {
    ...
    if (!isValid) error.textContent = "Invalid bet.";
    else {
      setTimeout(
        _ => $(el).hide(200, "linear", () => submitCb(+input.value)),
        200
      );
    }
  });
