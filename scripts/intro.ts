let elFirst = <HTMLInputElement>document.querySelector("#firstName");
let elLast = <HTMLInputElement>document.querySelector("#lastName");
let elUsername = <HTMLInputElement>document.querySelector("#username");
let elPhone = <HTMLInputElement>document.querySelector("#phoneNum");
let elPostal = <HTMLInputElement>document.querySelector("#pCode");
let elMoney = <HTMLInputElement>document.querySelector("#bankRoll");
let elForm = document.querySelector("form");

// Refreshing breaks state
let allInputs = <NodeListOf<HTMLInputElement>>(
  document.querySelector("form").querySelectorAll(".field input")
);
...
$("form").validate({
  ...
  messages: {
    firstName: {
      pattern:
        "First character must be letter and  May only contain letters, spaces, dashes and single quotes",
    ...
    username: {
      pattern:
        "Username must start with a lower case letter, followed by three digits and finally either an uppercase A or B",
    },
    phoneNum: {
      pattern:
...
elForm.addEventListener("submit", function (e) {
  ...
  let allInputs = <NodeListOf<HTMLInputElement>>(
    this.querySelectorAll(".field input")
  );
  [...allInputs].forEach((i) => i.dispatchEvent(new Event("change")));

  if (validInputs.length != allInputs.length) return e.preventDefault();
...
if (location.search != "?change") {
  ...
  if (complete) document.getElementById("btn").click();
} else {
